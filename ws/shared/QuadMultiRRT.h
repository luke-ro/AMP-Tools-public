...
class QuadMultiRRT{
    ...
    QuadMultiRRT(Eigen::Matrix<double, 4, 1> epsilon_vec, int N_MAX=10000, double Dt=0.2, double p_goal=0.05, double epsilon=0.25, double radius=0.5):
        _N_MAX (N_MAX),
        _Dt (Dt),
        _p_goal (p_goal),
        _epsilon (epsilon),
    ...
    double _epsilon;
    double _radius;
    Eigen::Matrix<double,4,1> _epsilon_vec;
